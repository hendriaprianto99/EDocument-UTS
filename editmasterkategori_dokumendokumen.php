...
<div id="page-wrapper">
    <?php
    //cek otoritas
    $q = "SELECT * FROM tw_hak_akses WHERE tabel='kategori_dokumen/dokumen' AND user = '" . $_SESSION['Email'] . "' AND editData='1'";
    ...
    ?>
        ...
        echo "<form action=editmasterkategori_dokumendokumenexec.php method=post>";
        $result = mysqli_query($con, "SELECT * FROM kategori_dokumen WHERE Kode = '" . $_GET['Kode'] . "'");
        while ($row = mysqli_fetch_array($result)) {
            echo "<td bgcolor=CCEEEE><input type=hidden name=pk value='" . $row['Kode'] . "'></td>";
            echo "<tr><td bgcolor=CCCCCC><font face=Verdana color=black size=1>&nbsp;&nbsp;Kode&nbsp;&nbsp;</font></td>";
            echo "<td bgcolor=CCEEEE><input type=text class='form-control' name='Kode' value='" . $row['Kode'] . "'></td>";
            echo "<tr><td bgcolor=CCCCCC><font face=Verdana color=black size=1>&nbsp;&nbsp;Kategori&nbsp;&nbsp;</font></td>";
            echo "<td bgcolor=CCEEEE><input type=text class='form-control' name='Kategori' value='" . $row['Kategori'] . "' size = 100></td>";
            echo "<tr><td bgcolor=CCCCCC><font face=Verdana color=black size=1>&nbsp;&nbsp;Keterangan&nbsp;&nbsp;</font></td>";
            echo "<td bgcolor=CCEEEE><textarea class='form-control' name='Keterangan' cols=70 rows=5>" . $row['Keterangan'] . "</textarea></td>";
