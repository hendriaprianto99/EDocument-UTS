    ...
    if ($obj = mysqli_fetch_object($r)) {
        ...
        <td bgcolor=F5F5F5 valign=top>
            <table class='table table-striped'>
                ...
                <form action=editdokumen_fileexec.php method=post>
                    <?php
                    $result = mysqli_query($con, "SELECT * FROM dokumen_file where id = " . mysqli_real_escape_string($con, $_GET['id']) . "");
                    while ($row = mysqli_fetch_array($result)) {
                        ?>
                        <tr>
                            <td colspan=2><input type=hidden name=pk value='<?php echo $row['id']; ?>'></td>
                        </tr>
                        <tr>
                        ...
                        <tr>
                            <td bgcolor=CCCCCC><font face=Verdana color=black size=1>&nbsp;&nbsp;id&nbsp;&nbsp;</font></td>
                            <td bgcolor=CCEEEE><font face=Verdana color=black size=1>&nbsp;&nbsp;auto_increment&nbsp;&nbsp;</font></td>
                        </tr>
                        <tr>
                            <td colspan=2><input type=hidden name=id value='<?php echo $row['id']; ?>'></td>
                        </tr>
                        <tr>
                            <td bgcolor=CCCCCC><font face=Verdana color=black size=1>&nbsp;&nbsp;No_Dokumen&nbsp;&nbsp;</font></td>
                            <td bgcolor=CCEEEE>
                                <select class='form-control' name='No_Dokumen'>
                                    <?php
                            ...
                            <?php if (isset($row['id'])) { ?>
                                <td bgcolor=CCEEEE>
                                    <?php if (!empty($row['File'])) { ?>
                                        <a href='files/<?php echo $row['File']; ?>' target=_blank>'<?php echo $row['File']; ?>'</a><br>
                                        <input type=text class='form-control' name=File value="<?php echo $row['File']; ?>" hidden>
                                    <?php } ?>
                                    <a href=uploadfiledokumen_file.php?id=<?php echo $row['id']; ?>>upload</a>
                                </td>
                            <?php } ?>
                        </tr>
                        <tr>
